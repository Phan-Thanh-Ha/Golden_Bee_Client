  ...
  return (
    <LayoutGradientBlue>
      ...
      <Text style={styles.subTitle}>
        Chúc mừng {userLogin?.OfficerName}, bạn đã hoàn thành đơn dịch vụ {data?.DataService?.ServiceName.toLowerCase()}
      </Text>

      <View style={{ backgroundColor: colors.primary[700], margin: 10, borderRadius: 10, paddingVertical: 20 }}>
        <View style={MainStyles.flexRowCenter}>
          <Text style={[MainStyles.titleCardJob, { textAlign: 'center' }]}>Dịch vụ {data?.DataService?.ServiceName.toLowerCase()}</Text>
        </View>
        {
          data?.BookingCode ? (
            <Text style={{ textAlign: 'center', fontSize: 12, color: colors.MAIN_COLOR_CLIENT, fontWeight: 'bold' }}>{data?.BookingCode}</Text>
          ) : null
        }
        <View style={MainStyles.flexRowCenter}>
      ...
      <Text style={styles.subTitle}>
        Cảm ơn bạn đồng hành và làm việc cùng chúng tôi. Hãy giữ sức khỏe để tiếp tục đồng hành cùng <Text style={styles.yellowText}>Ong Vàng</Text> nhé!
      </Text>
